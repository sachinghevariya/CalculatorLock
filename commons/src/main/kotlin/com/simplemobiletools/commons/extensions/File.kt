...
fun File.getDirectChildrenCount(context: Context, countHiddenItems: Boolean, isVideo: Boolean,isAudio:Boolean): Int {
    val fileCount = if (context.isRestrictedSAFOnlyRoot(path)) {
        context.getAndroidSAFDirectChildrenCount(
                path,
                countHiddenItems
        ...
        listFiles()?.count { file ->
            (file.isDirectory || (file.isFile && ((isAudio && isAudioFile(file))||(isVideo && isVideoFile(file)) || (!isVideo && isImageFile(file)))))
                    && (countHiddenItems || !file.name.startsWith('.'))
        } ?: 0
    }
    return fileCount
}
...
private fun isVideoFile(file: File): Boolean {
    val videoFileExtensions = listOf(".mp4", ".3gp", ".avi", ".mkv", ".mov", ".wmv")
    return videoFileExtensions.any { extension ->
        file.name.endsWith(extension, ignoreCase = true)
    }
}

private fun isImageFile(file: File): Boolean {
    val imageFileExtensions = listOf(".jpg", ".jpeg", ".png", ".bmp")
    return imageFileExtensions.any { extension ->
        file.name.endsWith(extension, ignoreCase = true)
    }
}

private fun isAudioFile(file: File): Boolean {
    val audioFileExtensions = listOf(".mp3", ".wav", ".ogg", ".aac", ".m4a")
