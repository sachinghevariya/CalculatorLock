...
open class FileDirItem(
    ...
    fun getDirectChildrenCount(context: Context, countHiddenItems: Boolean,isVideo :Boolean,isAudio:Boolean): Int {
        return when {
            ...
            context.isPathOnOTG(path) -> context.getDocumentFile(path)?.listFiles()?.filter { if (countHiddenItems) true else !it.name!!.startsWith(".") }?.size
                ?: 0
            else -> File(path).getDirectChildrenCount(context, countHiddenItems,isVideo,isAudio)
        }
    }
