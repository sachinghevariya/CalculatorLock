...
import calculator.lock.hide.photo.video.calculatorvault.safe.photovault.hideru.vault.utils.createMediaItemsFromFilePaths
...
class ItemsFragment(context: Context, attributeSet: AttributeSet) :
    MyViewPagerFragment<MyViewPagerFragment.ItemsInnerBinding>(context, attributeSet),
    ...
    private var selectedList = ArrayList<ListItem>()
        ...
        if (this.activity == null) {
            ...
            binding.apply {
                if (activity.isAudio) {
                    ...
                    dbFolderId = 2
                    binding.tvHideFolderName.text = "${context.getString(R.string.all_audio)}"
                } else if (activity.isVideo) {
                    dbFolderName = "My Videos"
                    dbFolderId = 1
                    binding.tvHideFolderName.text = "${context.getString(R.string.all_videos)}"
                } else {
                    dbFolderName = "My Photos"
                    dbFolderId = 0
                    binding.tvHideFolderName.text = "${context.getString(R.string.all_photos)}"
                }
        ...
        activity?.runOnUiThread {
            ...
            ItemsAdapter(
                ...
                if ((it as? ListItem)?.isSectionTitle == true) {
                    ...
                    itemClicked(it as ListItem, pos)
                }
            }.apply {
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    ...
                    withContext(Dispatchers.Main) {
                        if (mediaItems.isEmpty()) {
                            activity!!.showToast(activity!!.getString(R.string.select_media))
                        } else {
                            showMyDialog(
                                activity!!,
                                HideLoadingDialog(mediaItems, dbFolderName, dbFolderId) {
//                                    openPath(currentPath)
    ...
    private fun getRecyclerLayoutManager() =
        (binding.itemsList.layoutManager as MyGridLayoutManager)
            ...
            if (activity?.isDestroyed == false && activity?.isFinishing == false) {
                ...
                if (context.isRestrictedSAFOnlyRoot(path)) {
                    activity?.handleAndroidSAFDialog(path) {
                        if (!it) {
                            activity?.toast(com.simplemobiletools.commons.R.string.no_storage_permissions)
                            return@handleAndroidSAFDialog
                        }
                        val getProperChildCount =
                            context!!.config.getFolderViewType(currentPath) == VIEW_TYPE_LIST
                    ...
                    val getProperFileSize =
                        context!!.config.getFolderSorting(currentPath) and SORT_BY_SIZE != 0
        ...
        for (file in files) {
            ...
            if (listItem != null) {
                if (!listItem.isDirectory) {
                    if (activity!!.isAudio) {
                        if (audioMimeTypes.any {
                                checkMimeTypeForListItem(
                                    it,
                                    listItem
                                )
                            }) {
    ...
    fun isVideoFile(fileName: String): Boolean {
        val videoExtensions =
            arrayOf("mp4", "avi", "mkv", "mov", "wmv", "flv") // Add more video extensions if needed
        val fileExtension = fileName.substringAfterLast(".", "").toLowerCase()
        return videoExtensions.contains(fileExtension)
    }

    private fun checkMimeTypeForListItem(wantedMimeType: String, listItem: ListItem): Boolean {
        if (wantedMimeType.equals(getMimeType(listItem.mName), true)) {
            return true
        }
        return false
    }
    ...
    private fun getListItemsFromFileDirItems(fileDirItems: ArrayList<FileDirItem>): ArrayList<ListItem> {
        ...
        fileDirItems.forEach {
            ...
            if (wantedMimeTypes.any { mimeType ->
                    isProperMimeType(
                        mimeType,
                        it.path,
                        it.isDirectory
    ...
    private fun itemClicked(item: ListItem, pos: Int) {
        if (item.isDirectory) {
            ...
            getRecyclerAdapter()?.selectedItems = selectedList
            getRecyclerAdapter()?.notifyItemChanged(pos)
            manageUi()
//            clickedPath(item.path)
    ...
    private fun manageUi() {
        binding.tvHide.text = "${context.getString(R.string.hide)}(${selectedList.size})"
    }
    ...
    override fun searchQueryChanged(text: String) {
        ...
        binding.apply {
            itemsSwipeRefresh.isEnabled =
                text.isEmpty() && activity?.config?.enablePullToRefresh != false
            when {
                text.isEmpty() -> {
    ...
    private fun setupGridLayoutManager() {
        ...
        layoutManager.spanSizeLookup = object : GridLayoutManager.SpanSizeLookup() {
            override fun getSpanSize(position: Int): Int {
                return if (getRecyclerAdapter()?.isASectionTitle(position) == true || getRecyclerAdapter()?.isGridTypeDivider(
                        position
                    ) == true
                ) {
    ...
    private fun increaseColumnCount() {
        if (currentViewType == VIEW_TYPE_GRID) {
            ...
            (activity as? FoldersActivity)?.updateFragmentColumnCounts()
        }
    }
    ...
    override fun columnCountChanged() {
        (binding.itemsList.layoutManager as MyGridLayoutManager).spanCount =
...
//                    activity as SimpleActivity,
//                    currentPath,
//                    context!!.config.enableRootAccess,
//                    true
//            ) {
//                getRecyclerAdapter()?.finishActMode()
//                openPath(it)
...
//        }
    ...
    override fun selectedPaths(paths: ArrayList<String>) {
        (activity as FoldersActivity).pickedPaths(paths)
    }
}
