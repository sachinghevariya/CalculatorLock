...
import calculator.lock.hide.photo.video.calculatorvault.safe.photovault.hideru.vault.databinding.FragmentHideAudioBinding
...
@AndroidEntryPoint
class HideAudioFragment(private val isFromHide:Boolean = false, private val functionCallback: (isSelectionMode: Boolean) -> Unit) :
    BaseFragment<FragmentHideAudioBinding>() {
    override fun getViewBinding() = FragmentHideAudioBinding.inflate(layoutInflater)
    ...
    override fun init() {
        ...
        binding.rvAudio.layoutManager = LinearLayoutManager(requireActivity(), RecyclerView.VERTICAL,false)
        binding.cbAll.isEnabled = false

        audioAdapter =
            AudioAdapter(requireActivity() as AppCompatActivity, arrayListOf(),isFromHide = isFromHide, arrayListOf()) { list, selectedItem ->
        ...
        lifecycleScope.launch {
            viewModel.allAudioItemsFlow.observe(this@HideAudioFragment) {
                if (it.isNotEmpty()) {
        ...
        binding.btnMove.setOnClickListener(1000L) {
            showBottomSheet(requireActivity() as AppCompatActivity,
                BsHideFolderListDialog(true,isVideo = false, isAudio = true) { dbFolderName, dbFolderId ->
    ...
    private fun updateAudioList(isAllSelected: Boolean) {
        currentFolderList = mediaItems
        audioAdapter.selectedList = selectedList
        audioAdapter.isSelectionEnable = isAllSelected
